    ...
    <main class="flex-1 lg:ml-72 min-h-screen flex flex-col relative pb-20 lg:pb-0">
        ...
        <div class="p-container-padding-mobile lg:p-container-padding-desktop pt-0 grid grid-cols-12 gap-grid-gap flex-1">
            <div class="col-span-12 xl:col-span-4 space-y-grid-gap">
                <section class="glass-card tech-border tech-corner-tl p-6" id="quick-add">
                    ...
                    <form class="space-y-6" method="POST" action="{{ route('participants.store') }}">
                        @csrf
                        <div class="space-y-2">
                            <label class="font-label-sm text-[10px] text-on-surface-variant flex justify-between"><span>NOMOR_ID_NODE</span><span class="text-primary/40">[ID]</span></label>
                            <input class="glass-input w-full px-4 py-3 text-sm focus:ring-0" name="id_number" placeholder="MASUKKAN_NOMOR_ID..." required type="text" value="{{ old('id_number') }}">
                        </div>
                        <div class="space-y-2">
                            <label class="font-label-sm text-[10px] text-on-surface-variant flex justify-between"><span>IDENTIFIER_NAME</span><span class="text-primary/40">[STR]</span></label>
                            <input class="glass-input w-full px-4 py-3 text-sm focus:ring-0" name="name" placeholder="MASUKKAN_NAMA..." required type="text" value="{{ old('name') }}">
                        </div>
                        <div class="space-y-2">
                            <label class="font-label-sm text-[10px] text-on-surface-variant flex justify-between"><span>ADDRESS_NODE</span><span class="text-primary/40">[TXT]</span></label>
                            <textarea class="glass-input w-full px-4 py-3 text-sm focus:ring-0 min-h-24" name="address" placeholder="MASUKKAN_ALAMAT..." required>{{ old('address') }}</textarea>
                        </div>
                        <div class="space-y-2">
                            <label class="font-label-sm text-[10px] text-on-surface-variant flex justify-between"><span>SERIAL_ID_OR_CONTACT</span><span class="text-primary/40">[PHONE]</span></label>
                            <input class="glass-input w-full px-4 py-3 text-sm focus:ring-0" name="phone_number" placeholder="CONTOH: 081234567890" required type="text" value="{{ old('phone_number') }}">
                        </div>
                        <div class="space-y-2">
                            <label class="font-label-sm text-[10px] text-on-surface-variant flex justify-between"><span>SUBSCRIPTION_FEE</span><span class="text-primary/40">[IDR]</span></label>
                            <input class="glass-input w-full px-4 py-3 text-sm focus:ring-0" name="subscription_fee" placeholder="CONTOH: 50000" required type="number" value="{{ old('subscription_fee') }}">
                        </div>
                ...
                <section class="glass-card tech-border p-6 flex-1">
                    ...
                    <div id="dropZone" class="border-2 border-dashed border-white/10 bg-black/20 rounded p-8 flex flex-col items-center justify-center text-center cursor-pointer hover:border-secondary/50 hover:bg-secondary/5 transition-all">
                        ...
                        <p class="text-[10px] text-on-surface-variant/40 font-label-sm">Format: id_number, nama, alamat, no. telepon, biaya</p>
                    </div>
            ...
            <div class="col-span-12 xl:col-span-8 flex flex-col glass-card tech-border tech-corner-br overflow-hidden min-h-[620px]">
                ...
                <div class="hidden md:grid grid-cols-12 gap-4 px-8 py-3 bg-black/40 border-b border-white/5 font-label-sm text-[10px] text-on-surface-variant uppercase tracking-widest">
                    <div class="col-span-1">UID</div>
                    <div class="col-span-2">ID_NODE</div>
                    <div class="col-span-2">SUBJECT_NAME</div>
                    <div class="col-span-2">ADDRESS</div>
                    <div class="col-span-2">NETWORK_ID</div>
                    <div class="col-span-2">SUB_FEE</div>
                    <div class="col-span-1 text-right">OPS</div>
                </div>
                ...
                <div class="flex-1 overflow-y-auto custom-scrollbar bg-black/20">
                    @forelse ($participants as $participant)
                        <div class="data-row grid grid-cols-1 md:grid-cols-12 gap-3 md:gap-4 px-6 md:px-8 py-4 items-center">
                            ...
                            <div class="md:col-span-2 font-label-sm text-xs text-primary-container truncate">{{ $participant->id_number }}</div>
                            <div class="md:col-span-2 flex items-center gap-3 min-w-0">
                                <div class="w-7 h-7 tech-border flex items-center justify-center text-[10px] font-bold text-primary bg-primary/5 shrink-0">{{ strtoupper(substr($participant->name, 0, 1)) }}</div>
                                <span class="font-semibold text-on-surface text-sm truncate">{{ $participant->name }}</span>
                            </div>
                            <div class="md:col-span-2 font-label-sm text-xs text-on-surface-variant truncate">{{ $participant->address }}</div>
                            <div class="md:col-span-2 font-label-sm text-on-surface-variant text-xs truncate">{{ $participant->phone_number }}</div>
                            <div class="md:col-span-2 font-label-sm text-on-surface text-xs whitespace-nowrap">Rp {{ number_format($participant->subscription_fee, 0, ',', '.') }}</div>
                            <div class="md:col-span-1 md:text-right">
                                <button class="text-on-surface-variant hover:text-error transition-colors" type="button" onclick="openDeleteModal('{{ route('participants.destroy', $participant) }}', '{{ addslashes($participant->name) }}')">
